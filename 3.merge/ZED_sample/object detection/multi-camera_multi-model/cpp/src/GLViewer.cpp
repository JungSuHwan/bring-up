...
void GLViewer::updateObjects(
    ...
    for (const std::pair<sl::CameraIdentifier, std::unordered_map<unsigned int /* instance id */, sl::Objects>>& it : singledata) {
        ...
        if (BBox_edges_per_cam.find(it.first.sn) == BBox_edges_per_cam.end()) {
            BBox_edges_per_cam[it.first.sn] = Simple3DObject(sl::Translation(0, 0, 0), false);
            BBox_faces_per_cam[it.first.sn] = Simple3DObject(sl::Translation(0, 0, 0), false);
            BBox_edges_per_cam[it.first.sn].setDrawingType(GL_LINES);
            BBox_faces_per_cam[it.first.sn].setDrawingType(GL_QUADS);
        }
