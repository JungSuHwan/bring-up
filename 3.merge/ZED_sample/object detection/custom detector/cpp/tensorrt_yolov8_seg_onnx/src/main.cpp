...
static void draw_objects(cv::Mat const& image, cv::Mat& res, sl::Objects const& objs, std::vector<std::vector<int>> const& colors) {
    ...
    for (sl::ObjectData const& obj : objs.object_list) {
        ...
        const cv::Point text_position {bbox_center.x - label_size.width / 2, bbox_center.y - label_size.height / 2};
...
static std::vector<sl::uint2> convertCvRect2SdkBbox(cv::Rect_<float> const& bbox_in) {
    ...
    bbox_out.push_back(sl::uint2(static_cast<unsigned int>(bbox_in.x), static_cast<unsigned int>(bbox_in.y)));
    bbox_out.push_back(sl::uint2(static_cast<unsigned int>(bbox_in.x + bbox_in.width), static_cast<unsigned int>(bbox_in.y)));
    bbox_out.push_back(
        sl::uint2(static_cast<unsigned int>(bbox_in.x + bbox_in.width), static_cast<unsigned int>(bbox_in.y + bbox_in.height))
    );
    bbox_out.push_back(sl::uint2(static_cast<unsigned int>(bbox_in.x), static_cast<unsigned int>(bbox_in.y + bbox_in.height)));
    return bbox_out;
}
